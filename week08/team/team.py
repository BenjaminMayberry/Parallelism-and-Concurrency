...
def merge_sort(arr, TheadLimit = 0):
...
# -----------------------------------------------------------------------------
def merge_sort_thread(arr, TheadLimit = 0):
    ...
    if len(arr) > 1:
        ...
        x = threading.Thread(target=merge_sort, args=(L,))
        #merge_sort(L)
        x.start()
        x.join()
        # Sorting the second half
        y = threading.Thread(target=merge_sort, args=(R,))
        #merge_sort(R)
...
# -----------------------------------------------------------------------------
def merge_sort_process(arr, TheadLimit = 0):
    # TODO - Add your code here to use threads.  Each time the merge algorithm does a recursive
    #        call, you need to create a process to handle that call
    # TODO - Add your code here to use threads.  Each time the merge algorithm does a recursive
    #        call, you need to create a thread to handle that call
    if TheadLimit > 1:
        TheadLimit = TheadLimit - 1
        # Finding the mid of the array
        mid = len(arr) // 2
        ...
        x = threading.Thread(target=merge_sort_thread, args=(L,TheadLimit))
        #merge_sort(L)
        x.start()
        x.join()
        # Sorting the second half
        y = threading.Thread(target=merge_sort_thread, args=(R,TheadLimit))
        #merge_sort(R)
