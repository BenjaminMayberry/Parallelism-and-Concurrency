...
def run_with_threads(tur, log, main_turtle):
    ...
    # You need to use 4 threads where each thread concurrently drawing one type of shape.
    # You are free to change any functions in this code except main()
    s = threading.Thread(target=draw_squares, args=(tur, lock))
    c = threading.Thread(target=draw_circles, args=(tur,lock))
    t = threading.Thread(target=draw_triangles, args=(tur,lock))
    r = threading.Thread(target=draw_rectangles, args=(tur,lock))
