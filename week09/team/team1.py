...
number_of_bytes = 0
class pholosofers(threading.Thread):
  def __init__(self, Name, Fork_left, Fork_right):
    threading.Thread.__init__(self)
    self.Name = Name
    self.left = Fork_left
    self.right = Fork_right
    self.times_eaten = 0
    self.ate = False
  
  def got_forks(self):
    l = self.left.acquire(True, 1)
    r = self.right.acquire(True, 1)
    if l and r:
      return True
    elif l:
      self.left.release()
  ...
  def run(self):
    ...
    while number_of_bytes < MAX_MEALS:
      print(number_of_bytes)
      self.eating()
    
def main():
    forks = [threading.Lock(), threading.Lock(), threading.Lock() ,threading.Lock(), threading.Lock()]
    philosophers = ["Plato", "Aristotle", "Lutwig", "Calvin", "Hobbs"]
    allPholosifers = []
    for i in range(1,6):
      if i == 5:
        allPholosifers.append(pholosofers(philosophers[i - 1], forks[4], forks[0]))
      else:
        allPholosifers.append(pholosofers(philosophers[i - 1], forks[i], forks[i - 1]))
    for philoserfer in allPholosifers:
      philoserfer.start()
    for philoserfer in allPholosifers:
      philoserfer.join()
